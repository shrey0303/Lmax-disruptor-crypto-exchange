...
public class Balances {
    ...
    public void deposit(long id, long amount) {
        if (amount <= 0) {
            throw new shrey.common.exception.Exchange4xxException("Amount must be greater than 0");
        }
        Optional.ofNullable(balances.get(id))
            .orElseThrow(() -> new Exchange4xxException(String.format("Balance %s not found", id)))
            .increase(amount);
        captureBalance(id);
    }
    ...
    public void transfer(long fromId, long toId, long amount) {
        if (fromId == toId) {
            throw new Exchange4xxException("FromId and ToId must be different");
        }
        if (amount <= 0) {
            throw new Exchange4xxException("Amount must be greater than 0");
        }

        if (!balances.containsKey(fromId)) {
            throw new Exchange4xxException(String.format("Balance %s not found", fromId));
        }
        if (!balances.containsKey(toId)) {
            throw new Exchange4xxException(String.format("Balance %s not found", toId));
        }
